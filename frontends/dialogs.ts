...
export async function showDeviceInfo(serial: string) {
    ...
    try {
        ...
        const tempColor = tempVal > 40 ? 'text-orange-500' : 'text-slate-500';
      ...
      <div class="flex items-center gap-3 mb-5">
        ...
        <div>
          <div class="text-[15px] font-bold text-slate-900">${esc(i.brand)} ${esc(i.model)}</div>
          <div class="flex items-center gap-1.5 mt-0.5">
            <span class="inline-flex items-center gap-1 text-[11px] font-semibold text-slate-400">${typeIcon} ${typeLabel}</span>
            <span class="text-slate-200">·</span>
            <span class="text-[11px] font-mono text-slate-400">${esc(i.serial)}</span>
          </div>
        </div>
      </div>
      ...
      <div class="flex gap-2 mb-5">
        <div class="flex-1 px-3 py-2.5 rounded-lg bg-slate-50 border border-slate-100 text-center">
          <div class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">电量</div>
          <div class="text-[16px] font-bold ${batteryColor}">${batteryPct}%</div>
        </div>
        <div class="flex-1 px-3 py-2.5 rounded-lg bg-slate-50 border border-slate-100 text-center">
          <div class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">温度</div>
          <div class="text-[16px] font-bold ${tempColor}">${tempVal}°C</div>
        </div>
        <div class="flex-1 px-3 py-2.5 rounded-lg bg-slate-50 border border-slate-100 text-center">
          <div class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Android</div>
          <div class="text-[14px] font-bold text-slate-700">${esc(i.android_version)}</div>
        </div>
      </div>

      <!-- Detail Rows -->
      <div class="flex flex-col gap-0 rounded-lg border border-slate-100 overflow-hidden">
        <div class="flex items-center justify-between px-3.5 py-2.5 bg-white border-b border-slate-100">
          <span class="text-[11px] font-semibold text-slate-400 uppercase tracking-wide">序列号</span>
          <span class="text-[12px] font-mono font-medium text-slate-700">${esc(i.serial)}</span>
        </div>
        <div class="flex items-center justify-between px-3.5 py-2.5 bg-slate-50 border-b border-slate-100">
          <span class="text-[11px] font-semibold text-slate-400 uppercase tracking-wide">品牌</span>
          <span class="text-[12px] font-medium text-slate-700">${esc(i.brand)}</span>
        </div>
        <div class="flex items-center justify-between px-3.5 py-2.5 bg-white border-b border-slate-100">
          <span class="text-[11px] font-semibold text-slate-400 uppercase tracking-wide">型号</span>
          <span class="text-[12px] font-medium text-slate-700">${esc(i.model)}</span>
        </div>
        <div class="flex items-center justify-between px-3.5 py-2.5 bg-slate-50 border-b border-slate-100">
          <span class="text-[11px] font-semibold text-slate-400 uppercase tracking-wide">SDK 版本</span>
          <span class="text-[12px] font-mono font-medium text-slate-700">${esc(i.sdk_version)}</span>
        </div>
        <div class="flex items-center justify-between px-3.5 py-2.5 bg-white">
          <span class="text-[11px] font-semibold text-slate-400 uppercase tracking-wide">分辨率</span>
          <span class="text-[12px] font-mono font-medium text-slate-700">${esc(i.display_resolution)}</span>
        </div>
      </div>`;
    } catch (e) {
